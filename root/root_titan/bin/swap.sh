...
startSwapinfo()
{
  case $debug in on|full)
    echo "$CMD [$INPUT] SWAP mount swapfile ($retstr)";;
  esac
  case $aktion in
    showInfoDirect)
...
startMountramswap()
{
  ...
  case $debug in
    on|full)
      echo "$CMD [$INPUT] start startMountramswap";;
  esac

  case $model in
    fs9000|hs7110|hs7119|hs7420|hs7429|hs7810a|hs7819|hs8200|ufs912|ufs913|spark|spark7162)
      echo "$CMD [$INPUT] skip Load ramswap compress/decompress";;
    *)
      echo "$CMD [$INPUT] Load ramswap compress/decompress"
      if [ -e /lib/modules/lzo1x_compress.ko ]; then
...
#	echo disksize: $disksize
  ...
  if [ -e /lib/modules/ramzswap.ko ]; then
    case $model in
      ufs910|hs9510|fs9000)
...
startUmountramswap()
{
  ...
  case $debug in
    on|full)
      echo "$CMD [$INPUT] start startUmountramswap";;
  esac
...
startCreateswapfile()
{
  case $debug in
    on|full)
      echo "$CMD [$INPUT] start startCreateswapfile";;
  esac
  size10=$(($1/10))
  rest=$(($1-($size10*10)))
  killall -stop rcS
...
startUmountswapfile()
{
  case $aktion in
    loop)
      swapfile=`losetup -a | grep -m1 swapdir/swapfile`
      fastcut "$swapfile" ":" 1
      swapfile="$fcr"
    ...
    *)
      if ! grep -q swapdir/swapfile /proc/swaps; then
        return
      fi
      swapfile="$swapdir/swapfile";;
  esac
  case $debug in
    on|full)
      echo "$CMD [$INPUT] start startUmountswapfile";;
  esac
  swapoff "$swapfile"
  case $aktion in
    loop) losetup -d "$swapfile";;
  esac
  #echo 60 > /proc/sys/vm/swappiness
}
...
startSwapfile()
{
  ...
  if [ ! -e "$swapdir/swapfile" ]; then
    case $debug in
      on|full)
        echo "SWAP create $swapsize KB swapfile";;
    esac
...
startUmountswappart()
{
  if [ -e /tmp/.swappartdev ]; then
    read swappartdev < /tmp/.swappartdev
    fastcut "$swappartdev" "#" 1
    swappartdev="$fcr"
  else
    return
  fi
