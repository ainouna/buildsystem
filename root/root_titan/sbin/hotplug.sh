...
startRepairdev()
{
  ...
  case $? in
    ...
    *)
      killall -stop rcS
      killall -stop "$SWTYPE"			
      infobox 9999 "INFO" "The device /dev/$device could not be mounted" "Do a Filesystemcheck?" "In 30 Second a Filesystemcheck will be started automatically" "" "OK = Do Filesystemcheck" "EXIT = No Filesystemcheck" &
...
startStickextensions() {
  ...
  if [ ! -e "$1/usr/local/share/titan/saver" ]; then
    mkdir -p "$1/usr/local/share/titan/saver"
  fi
  if [ ! -e "$1/usr/local/share/titan/picons" ]; then
    mkdir -p "$1/usr/local/share/titan/picons"
  fi
  if [ ! -e "$1/usr/local/share/titan/skin" ]; then
    mkdir -p "$1/usr/local/share/titan/skin"
  fi
  if [ ! -e "$1/usr/local/share/titan/plugins" ]; then
    mkdir -p "$1/usr/local/share/titan/plugins"
  fi
  if [ ! -e "$1/usr/local/share/titan/imdbfolder" ]; then
   mkdir -p "$1/usr/local/share/titan/imdbfolder"
  fi
    ...
    *)
      ...
      case $? in 
        0)
          checkdev=0;;
        *)
          echo "[$0] startCheckdev: cannot mount device=$device label=$label type=$type" >> $LOG
          sync
    ...
    echo "[$0] startCheckdev: cannot check ... automount is not running" >> $LOG
    sync
    checkdev=1
  fi
}
...
case $startmode in
  first)
    ...
    for ROUND in $sdlist; do
      if [ -e $MAINDIR/$ROUND ]; then
        sdxlist="$sdxlist $ROUND"
      fi
    done
...
case $ACTION in
  add|"")
    ...
    if [ ! -e /etc/.oebuild ]; then
      if [ "${device:0:8}" != "mmcblk" ] && [ ${#device} -ne 4 ]; then
        exit
      fi
    fi
    if [ ! -e /etc/.oebuild ]; then
      startWaitfor "$TMPDIR/.hotplugready" 1000
      rm -f $TMPDIR/.hotplugready
    fi
    ...
    if [ -e $TMPDIR/.swapdirdev ] || [ -e $TMPDIR/.swappartdev ]; then
      swapdir=1
    fi
