...
if [ "$keepsettings" == "1" ]; then
  ...
  if [ ! -d /instsrc/settings ] || [ ! -f /instsrc/settings/backup.tar.gz ]; then
	...
	echo
    echo "Saving current settings to backup-new.tar.gz"
    echo "but will use backup.tar.gz to restore."
  fi
...
#  chmod -R u=rwx,g=rw,o=r . 
  ...
  sync
  umount /instdest
  echo "<- Installing root file system finished ->"
else
...
# Make sure that the data partition contains required subdirectories
mkdir -p /mnt
mount $DATAFS /mnt
mkdir -p /mnt/movie
mkdir -p /mnt/music
