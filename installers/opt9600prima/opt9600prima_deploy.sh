...
echo
echo " Version for Opticum HD (TS) 9600 PRIMA"
#echo
...
if [ -z "`cat /proc/mtd | grep mtd3 | grep 'kernel'`" ]
then
  echo "MTD ERR" > /dev/vfd
  echo "MTD ERROR "
  exit 1
fi
...
if [ $format == "1" ]; then
  ...
  if [ "$usejfs" == "1" ]; then
    ...
    if [ "$useext2e2" == "1" ]; then
       echo "5 FRM E2"  > /dev/vfd
    else
       echo "5 FRM E3"  > /dev/vfd
    fi
...
if [ "$keepbootargs" != "1" ]; then
  ...
  echo "3 BOOTARGS" > /dev/vfd
#  if [ "$usbhdd" == "1" ]; then
#    dd if=/deploy/U-Boot_Settings_usb.mtd1 of=/dev/mtdblock1 2> /dev/null
#  else
#    dd if=/deploy/U-Boot_Settings_hdd.mtd1 of=/dev/mtdblock1 2> /dev/null
#  fi
#  if [ $? -ne 0 ]; then
#    echo "FAIL" > /dev/vfd
#    exit
#  fi
#  /usr/sbin/fw_setenv bootcmd_4 'run bootargs_6; bootm a0040000'
#  sleep 1
#  /usr/sbin/fw_setenv bootargs_4 'set bootargs console=ttyAS0,115200 root=/dev/sda1 rw ip=192.168.178.119:192.168.178.101:192.168.178.1:255.255.255.0:kathrein:eth0:off mem=256m coprocessor_mem=4m@0x40000000,4m@0x40400000 rootdelay=3 nwhwconf=device:eth0,hwaddr:${ethaddr} init=/bin/devinit'
#  sleep 1
#  /usr/sbin/fw_setenv bootdesc_4 'HDD'
#  sleep 1
#  /usr/sbin/fw_setenv boot_default '4'
#  sleep 1
  echo "done ->"
else
...
dd if=/instsrc/uImage.flash of=/dev/mtdblock3 # 2> /dev/null
if [ $? -ne 0 ]; then
