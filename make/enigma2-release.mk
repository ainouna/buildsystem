...
enigma2_release_spark7162:
	...
	mkdir -p $(RELEASE_DIR)/var/run/lirc
	cp -f $(SKEL_ROOT)/release/rc_spark.png $(RELEASE_DIR)/usr/local/share/enigma2/skin_default/rc.png
	if [ -e $(TARGET_DIR)/usr/lib/enigma2/python/Plugins/Extensions/spark7162VFD/plugin.py ]; then \
		rm -f $(RELEASE_DIR)/usr/lib/enigma2/python/Plugins/SystemPlugins/VFD-Icons/*; \
...
#
	...
	[ -e $(TARGET_DIR)/lib/modules/$(KERNEL_VER)/extra/wireless/rtl8192eu/8192eu.ko ] && cp $(TARGET_DIR)/lib/modules/$(KERNEL_VER)/extra/wireless/rtl8192eu/8192eu.ko $(RELEASE_DIR)/lib/modules/ || true
ifeq ($(IMAGE), $(filter $(IMAGE), enigma2-wlandriver neutrino-wlandriver))
	...
	[ -e $(SKEL_ROOT)/etc/wpa_supplicant.wlan0.conf ] && cp -aR $(SKEL_ROOT)/etc/wpa_supplicant.wlan0.conf $(RELEASE_DIR)/etc/ || true
endif
#
# lib usr/lib
#
...
#
	...
	cp -f $(SKEL_ROOT)/root_enigma2/usr/local/share/enigma2/skin_default/spinner/* $(RELEASE_DIR)/usr/local/share/enigma2/skin_default/spinner/
...
# DVB-T USB
#
	if [ -e $(TARGET_DIR)/lib/modules/$(KERNEL_VER)/kernel/drivers/media/dvb/frontends/dvb-pll.ko ]; then \
		cp $(TARGET_DIR)/lib/modules/$(KERNEL_VER)/kernel/drivers/media/dvb/frontends/dvb-pll.ko $(RELEASE_DIR)/lib/modules/; \
	fi
	...
	if [ -e $(TARGET_DIR)/lib/modules/$(KERNEL_VER)/extra/dvbt/it913x/dvb-it913x.ko ]; then \
