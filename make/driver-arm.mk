...
ifeq ($(BOXTYPE), hd51)
	$(START_BUILD)
	$(SILENT)install -d $(TARGET_DIR)/lib/modules/$(KERNEL_VER)/extra
...
$(D)/platform_util: $(D)/bootstrap $(ARCHIVE)/$(UTIL_SRC)
	$(START_BUILD)
	$(UNTAR)/$(UTIL_SRC)
	$(SILENT)install -m 0755 $(BUILD_TMP)/platform-util-$(KERNEL_TYPE)/* $(TARGET_DIR)/usr/bin
...
$(D)/libgles: $(D)/bootstrap $(ARCHIVE)/$(GLES_SRC)
	$(START_BUILD)
	$(UNTAR)/$(GLES_SRC)
	$(SILENT)install -m 0755 $(BUILD_TMP)/libgles-$(KERNEL_TYPE)/lib/* $(TARGET_DIR)/usr/lib
	$(SILENT)ln -sf libv3ddriver.so $(TARGET_DIR)/usr/lib/libEGL.so
	$(SILENT)ln -sf libv3ddriver.so $(TARGET_DIR)/usr/lib/libGLESv2.so
	$(SILENT)cp -a $(BUILD_TMP)/libgles-$(KERNEL_TYPE)/include/* $(TARGET_DIR)/usr/include
...
$(D)/vmlinuz_initrd: $(D)/bootstrap $(ARCHIVE)/$(INITRD_SRC)
	$(START_BUILD)
	$(SILENT)tar -xf $(ARCHIVE)/$(INITRD_SRC) -C $(TARGET_DIR)/boot
	$(SILENT)install -d $(TARGET_DIR)/boot
	$(TOUCH)
endif
