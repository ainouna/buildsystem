...
$(D)/enigma2.do_prepare: | $(ENIGMA2_DEPS)
	...
	if [ "$$DIFF" != "1" ]; then \
		...
		echo "Diff       : "$$DIFF; \
		echo; \
		[ -d "$(ARCHIVE)/enigma2-pli-nightly83.git" ] && \
		...
		[ -d "$(ARCHIVE)/enigma2-pli-nightly83.git" ] || \
		(echo -n "Cloning remote OpenPLi git..."; git clone -q -b $$HEAD_0 $$REPO_0 $(ARCHIVE)/enigma2-pli-nightly83.git; echo " done."); \
		echo -n "Copying local git content to build environment..."; cp -ra $(ARCHIVE)/enigma2-pli-nightly83.git $(SOURCE_DIR)/enigma2; echo " done."; \
