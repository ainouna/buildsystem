...
ifeq ($(BOXTYPE), $(filter $(BOXTYPE), atevio7500 fortis_hdbox octagon1008 ufs910 ufs912 ufs913 ufs922 ufc960 spark spark7162 ipbox55 ipbox99 ipbox9900 cuberevo cuberevo_mini cuberevo_mini2 cuberevo_250hd cuberevo_2000hd cuberevo_3000hd adb_box tf7700 vitamin_hd5000))
	...
	install -m 0755 $(SKEL_ROOT)/release/rcS_tvheadend_$(BOXTYPE) $(RELEASE_DIR)/etc/init.d/rcS
#
# player
#
...
#
	...
	cp $(TARGET_DIR)/usr/local/bin/tvheadend $(RELEASE_DIR)/usr/local/bin/
	[ ! -d $(RELEASE_DIR)/.hts/tvheadend ] && mkdir -p $(RELEASE_DIR)/.hts/tvheadend 
#	cp -r $(SKEL_ROOT)/root_tvheadend/.hts/tvheadend/* $(RELEASE_DIR)/.hts/tvheadend
...
#	default for all receivers
	...
	find $(OWN_BUILD)/tvheadend/ -mindepth 1 -maxdepth 1 -exec cp -at$(RELEASE_DIR)/ -- {} +
	echo " done."
#	receiver specific (only if directory exists)
	echo -n "Processing receiver specific own build for $(BOXTYPE)..."
	[ -d "$(OWN_BUILD)/tvheadend.$(BOXTYPE)" ] && find $(OWN_BUILD)/tvheadend.$(BOXTYPE)/ -mindepth 1 -maxdepth 1 -exec cp -at$(RELEASE_DIR)/ -- {} + || true
	echo " done."
