...
$(TARGET_DIR)/lib/libc.so.6:
	$(SILENT)if test -e $(CROSS_BASE)/$(TARGET)/sys-root/lib; then \
...
$(ARCHIVE)/$(CROSSTOOL_NG_SOURCE):
	$(SILENT)$(SCRIPTS_DIR)/get-git-archive.sh $(CROSSTOOL_NG_URL) $(CROSSTOOL_NG_VER) $(notdir $@) $(ARCHIVE)
...
crosstool-ng: directories $(ARCHIVE)/$(KERNEL_SRC) $(ARCHIVE)/$(CROSSTOOL_NG_SOURCE)
	...
	$(SILENT)unset CONFIG_SITE LIBRARY_PATH CPATH C_INCLUDE_PATH PKG_CONFIG_PATH CPLUS_INCLUDE_PATH INCLUDE; \
	set -e; cd $(BUILD_TMP)/crosstool-ng-$(CROSSTOOL_NG_VER); \
		...
		./configure $(SILENT_CONFIGURE) --enable-local; \
		MAKELEVEL=0 make; \
		chmod 0755 ct-ng; \
		./ct-ng oldconfig; \
		./ct-ng build
	$(SILENT)chmod -R +w $(CROSS_BASE)
	$(SILENT)test -e $(CROSS_BASE)/$(TARGET)/lib || ln -sf sys-root/lib $(CROSS_BASE)/$(TARGET)/
