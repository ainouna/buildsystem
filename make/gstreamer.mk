...
$(D)/gst_plugins_base: $(D)/bootstrap $(D)/libglib2 $(D)/orc $(D)/gstreamer $(D)/alsa_lib $(D)/libogg $(D)/libvorbis $(ARCHIVE)/$(GST_PLUGINS_BASE_SOURCE)
	...
	$(SILENT)for i in `cd $(TARGET_DIR)/usr/lib/gstreamer-1.0; echo *.la`; do \
		$(REWRITE_LIBTOOL_V)/gstreamer-1.0/$$i; done
...
GST_LIBAV_SOURCE = gst-libav-$(GSTREAMER_LIBAV_VERSION).tar.xz
GST_LIBAV_PATCH  = gst-libav-$(GSTREAMER_LIBAV_VERSION)-disable-yasm-for-libav-when-disable-yasm.patch
GST_LIBAV_PATCH += gst-libav-$(GSTREAMER_LIBAV_VERSION)-fix-sh4-compile-gcc48.patch
